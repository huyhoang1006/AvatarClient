...
@export_group("Tốc độ")
## Tốc độ đi bộ, px/giây. Cũng là mốc để tính tốc độ animation.
...
## Frame 0 của walk_* trùng pose đứng yên, nhấp nhẹ sẽ thấy trượt chân.
...
func current_move_speed() -> float:
	return float(run_speed if Input.is_action_pressed("run") else speed)
...
func apply_walk_bob() -> void:
	...
	# roundf để bám lưới pixel, không có nó nhân vật sẽ rung lăn tăn
